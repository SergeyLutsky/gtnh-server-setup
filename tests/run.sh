...
assert_success "seven-mod catalogue with retired-mod migration validates" mod_catalogue_validate "$mod_catalogue"
all_mods="$(mod_resolve "$mod_catalogue" 2.8.4 all)"
assert_eq "7" "$(jq length <<<"$all_mods")" "all seven offered mods resolve for GTNH 2.8.4"
assert_eq "7" "$(jq '[.[].artifact.sha256]|unique|length' <<<"$all_mods")" "mod artifacts have unique pinned checksums"
