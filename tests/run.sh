...
if python3 - "$TEST_ROOT/bin/gtnh-rcon.py" <<'PY'
import importlib.util, struct, sys
spec = importlib.util.spec_from_file_location("gtnh_rcon", sys.argv[1])
module = importlib.util.module_from_spec(spec)
spec.loader.exec_module(module)
body = struct.pack("<ii", 2, 0) + b"fragmented response\0\0"
wire = struct.pack("<i", len(body)) + body
class FragmentedSocket:
    def __init__(self, data): self.data = bytearray(data)
    def recv(self, size):
        take = min(1, size, len(self.data))
        result = bytes(self.data[:take])
        del self.data[:take]
        return result
request_id, kind, message = module.receive(FragmentedSocket(wire))
assert (request_id, kind, message) == (2, 0, "fragmented response")
PY
then
  pass "RCON framing accepts fragmented TCP headers and bodies"
else
  fail "RCON framing accepts fragmented TCP headers and bodies"
fi
...
assert_success "eight-mod catalogue with retired-mod migration validates" mod_catalogue_validate "$mod_catalogue"
all_mods="$(mod_resolve "$mod_catalogue" 2.8.4 all)"
assert_eq "8" "$(jq length <<<"$all_mods")" "all eight offered mods resolve for GTNH 2.8.4"
assert_eq "8" "$(jq '[.[].artifact.sha256]|unique|length' <<<"$all_mods")" "mod artifacts have unique pinned checksums"
...
ore_excavation="$(jq -c '.[]|select(.id=="ore-excavation")' <<<"$all_mods")"
iron_furnaces="$(jq -c '.[]|select(.id=="iron-furnaces")' <<<"$all_mods")"
not_enough_wands="$(jq -c '.[]|select(.id=="not-enough-wands")' <<<"$all_mods")"
...
assert_eq "1.1.134" "$(jq -r .version <<<"$ore_excavation")" "Ore Excavation uses the latest Minecraft 1.7.10 release"
assert_eq "OreExcavation-1.1.134.jar" "$(jq -r .artifact.name <<<"$ore_excavation")" "Ore Excavation resolves the pinned server and client artifact"
assert_eq '["OreExcavation-*.jar"]' "$(jq -c '.serverModFilePatterns[]|select(.id=="ore-excavation")|.patterns' "$TEST_ROOT/catalog/client-addons.json")" "client setup replaces other Ore Excavation versions"
assert_eq "1.2.4" "$(jq -r .version <<<"$iron_furnaces")" "Iron Furnaces uses the latest Minecraft 1.7.10 release"
assert_eq "ironfurnaces-1.2.4R.jar" "$(jq -r .artifact.name <<<"$iron_furnaces")" "Iron Furnaces resolves the pinned server and client artifact"
assert_eq '["ironfurnaces-*.jar"]' "$(jq -c '.serverModFilePatterns[]|select(.id=="iron-furnaces")|.patterns' "$TEST_ROOT/catalog/client-addons.json")" "client setup replaces other Iron Furnaces versions"
...
mkdir -p "$mod_validation_dir/stage"
assert_success "Not Enough Wands managed configuration is written" mods_apply_managed_configs "[$not_enough_wands]" "$mod_validation_dir/stage"
assert_success "Not Enough Wands managed configuration validates" mods_validate_managed_configs "[$not_enough_wands]" "$mod_validation_dir/stage"
assert_eq "40" "$(grep -Ec '_(needsxp|needsrf|maxrf|maxdurability)=0$' "$mod_validation_dir/stage/config/notenoughwands.cfg")" "all Not Enough Wands usage costs are disabled"
assert_eq "9" "$(grep -Ec 'I:item\.(Acceleration|Building|Capturing|Displacement|Illumination|Moving|Protection|Swapping|Teleportation)Wand_lootRarity=[1-9][0-9]*$' "$mod_validation_dir/stage/config/notenoughwands.cfg")" "normal Not Enough Wands chest loot remains enabled"
...
printf '%s\n' '#!/usr/bin/env bash' \
  ...
  'if [[ "$*" == "command bqapi list" ]]; then count=$(<"$GTNH_TEST_COMMAND_COUNTER"); count=$((count+1)); printf "%s\n" "$count" >"$GTNH_TEST_COMMAND_COUNTER"; ((count>=2)) && printf "%s\n" "GTNotLeisure75SteamAge"; ((count>=2)); exit; fi' \
...
export GTNH_SYSTEM_ROOT="$post_start_dir/system" GTNH_TEST_LOG_FILE="$post_start_dir/install/logs/latest.log" \
  GTNH_TEST_RCON_COUNTER="$post_start_dir/rcon-counter" GTNH_TEST_COMMAND_COUNTER="$post_start_dir/command-counter" \
  GTNH_POST_START_LOG_TIMEOUT=1 GTNH_POST_START_RCON_ATTEMPTS=2 GTNH_POST_START_RCON_RETRY_SECONDS=0 \
  GTNH_POST_START_COMMAND_ATTEMPTS=2 GTNH_POST_START_COMMAND_RETRY_SECONDS=0
: >"$GTNH_TEST_LOG_FILE"
...
chmod 0755 "$post_start_dir/system/usr/local/bin/gtnh"
assert_failure "quest reload rejects a stale log marker" mods_validate_post_start "$log_only_resolved" gtnh
unset GTNH_SYSTEM_ROOT GTNH_TEST_LOG_FILE GTNH_TEST_RCON_COUNTER GTNH_TEST_COMMAND_COUNTER \
  GTNH_POST_START_LOG_TIMEOUT GTNH_POST_START_RCON_ATTEMPTS GTNH_POST_START_RCON_RETRY_SECONDS \
  GTNH_POST_START_COMMAND_ATTEMPTS GTNH_POST_START_COMMAND_RETRY_SECONDS
