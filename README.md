...
The original five offered mods were tested together on GTNH 2.8.4 and reached the ready state. Ore Excavation 1.1.134 is pinned from its Minecraft 1.7.10 CurseForge release for both server and client installation. GTNH Rates is retired: it is never offered for new installations, and the next managed update removes its previously installed JAR after creating the verified backup. Selected add-ons must also be installed on each client; the installer prints the exact client list.
